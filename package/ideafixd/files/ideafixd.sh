# ideafixd API: (based on procd API)
#
# procd_open_service(name, [script]):
#   Initialize a new procd command message containing a service with one or more
#   instances
#
# procd_close_service()
...
#   Add an instance to the service described by the previous procd_open_service
#   call
#
# procd_set_param(type, [value...])
#   Available types:
#     command: command line (array).
#     respawn: array with 3 values $restart_timeout $fail_threshold $max_fail
