...
u32 ar71xx_sw_reg_read(struct mii_bus *mii, u32 reg_addr)
{
	u32 ret;
...
void ar71xx_sw_reg_write(struct mii_bus *mii, u32 reg_addr, u32 reg_val)
{
	mutex_lock(&reg_mutex);
	__ar7240sw_reg_write(mii, reg_addr, reg_val);
	mutex_unlock(&reg_mutex);
...
ar7240_hw_apply(struct switch_dev *dev)
{
	...
	if (as->vlan) {
		...
		/* don't isolation lan ports between each other for ap123, added by tomorrow */
		if(soc_is_ar9341()){
			int all_portmask = 0;
			for (i = 0; i < as->swdev.ports; i++) {
				all_portmask |= (1<<i);
...
void athrs27_phy_setup(struct ag71xx *ag)
{
    int       phyUnit;
	...
	for (phyUnit=0; phyUnit < AR7240_NUM_PHYS; phyUnit++) 
		{
            ar7240sw_phy_write(ag->mii_bus,phyUnit, ATHR_AUTONEG_ADVERT,ATHR_ADVERTISE_ALL);
		ar7240sw_phy_write(ag->mii_bus,phyUnit, ATHR_PHY_CONTROL,ATHR_CTRL_AUTONEGOTIATION_ENABLE|ATHR_CTRL_SOFTWARE_RESET);
    }
}
...
static struct ar7240sw *ar7240_probe(struct ag71xx *ag)
{
	...
	if (sw_is_ar7240(as)) {
		...
		if (as->swdata->phy4_mii_en) {
			...
			swdev->ports = AR7240_NUM_PORTS - 1;
			if (soc_is_ar9341()){
			ar7240sw_phy_write(mii, 4, 0, 0x9000);
			ar71xx_sw_reg_write(mii, 0x600, 0);

			/* Enable MDIO Access if PHY is Powered-down */
			ar7240sw_phy_write(mii,4,29,0x3);
			rd_data = ar7240sw_phy_read(mii,4,30);
			ar7240sw_phy_write(mii,4,29,0x3);
			ar7240sw_phy_write(mii,4,30,(rd_data & 0xfffffeff));

			ar71xx_sw_reg_write(mii,0X104,((1<<14) | 04));

			athrs27_phy_setup(ag);
			}
...
static void link_function(struct work_struct *work) 
{
	...
	struct ag71xx *ag_swap = NULL; 
	struct ar7240sw *as = ag->phy_priv;
	unsigned long flags;
	u8 mask;
	int i = 0,j = 0,swap_en = 0, wan_en = 0;
	int link,speed = 0,spec_status,status2 = 0, status = 0;

	swap_en = as->swdata->swap_en;
	wan_en = as->swdata->wan_en;
	...
	for (i = 0; i < AR7240_NUM_PHYS; i++)
	{
		...
		if(soc_is_ar9341())
		{
			if((!swap_en && (i == 4)) || (swap_en && (i == 0)))
			{
				if (wan_en)
				{
					ag_swap = (struct ag71xx *)ag->p_ag_data;
					spec_status = ar7240sw_phy_read(ag->mii_bus, i, PHY_SPEC_STATUS);
					speed = (spec_status & PHY_SPEC_STATUS_LINK_MASK) >> PHY_SPEC_STATUS_LINK_SHIFT;
					if(speed)
						ag_swap->speed = SPEED_100;
					else
						ag_swap->speed = SPEED_10;
					ag_swap->duplex = 0;
					if(spec_status & PHY_SPEC_STATUS_DUPLEX)
					{
						ag_swap->duplex = 1;
					}
	
					spin_lock_irqsave(&ag_swap->lock, flags);
					if (status != ag_swap->link)
					{
						ag_swap->link = status;
						ag71xx_link_adjust(ag_swap);
					}

					spin_unlock_irqrestore(&ag_swap->lock, flags);
				}
				else if(status)
				{
					status2++;
