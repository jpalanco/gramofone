...
generate_macaddr() {
	local base="$(cat /sys/class/ieee80211/${dev}/macaddress)"
	local offset="$1"
	local IFS=':'
	local num=''
	local mac=''
	...
	mac=`printf "%012x" $((0x0$num + $offset))`
	local IFS=''
	while [ $k -lt 6 ]
	do
		if [ $k -lt 1 ]; then
...
generate_suffix() {
	local suffix="$(generate_macaddr $1)"              
	suffix=${suffix:9:17}                             
	suffix=${suffix//:/}                                       
	printf "%s" $suffix
}
...
enable_mac80211() {
	...
	[ -n "$start_hostapd" ] && {
		# Optimization for fon2415: assume hostapd starts successfully, since
		# the opposite is a very rare situation and returning on failure does
		# not really contribute to recover. Also remove the 2 seconds wait that
		# has been tested unnecessary if interface up/down state is handled
		# completely by hostapd. This saves us a valuable time when changing
		# between networking modes.
		...
		# Optimization for fon2415: we don't need to up the interface here since
		# hostapd is handling this itself. Taking this out solves a bug where
		# wifi interfaces are eventually torn down by hostapd when it is not
		# able to complete its setup before this point due to the interfaces
		# coming up when hw mode or frequency settings are still pending.
		# Nevertheless, mac80211_start_vif is needed in order to fill the
		# network.interface netifd statuses, so it will be performed at the end.
		for vif in $vifs; do
			config_get mode "$vif" mode
			config_get ifname "$vif" ifname
			[ "$mode" = "ap" ] || continue
	...
	for vif in $vifs; do
		...
		# Optimization for fon2415: don't evaluate switch options when mode is
		# ap since it is not included. Make "sta" the first option since we only
		# have ap and sta mode.
		[ "$mode" = "ap" ] && continue
	...
	# Optimization for fon2415: do mac80211_start_vif for APs at the end so that
	# we move the necessary delay after the setup of the interfaces is done,
	# since we only need this to fill netifd configuration structures. This way
	# this delay is not added to the networking mode change process.
	sleep 2
	for vif in $vifs; do
		config_get mode "$vif" mode
		config_get ifname "$vif" ifname
...
config wifi-iface private$devidx
	option ifname	wpriv$devidx
	option device   radio$devidx
	option network  lan
	option mode     ap
	option ssid		Gramofon_$(generate_suffix 0)
	option encryption psk2
	option key	supersecret
	option macaddr	$(generate_macaddr 1)
	option wps_pushbutton 1
	option wps_device_name 'Gramofon'
	option wps_manufacturer 'Fon'

config wifi-iface-off hotspot$devidx
	option ifname	wpub$devidx
	option device radio$devidx
	option network fonopen
	option mode ap
	option ssid 'Fon WiFi$([ -n "$devidx" ] && echo " ${devidx}GHz")' 
	option isolate 1
	option encryption none
	option macaddr	$(generate_macaddr 0)

config wifi-iface-off hotspoteap$devidx
	option ifname	weap$devidx
	option device radio$devidx
	option network foneap
	option mode ap
	option ssid 'Fon Auto$([ -n "$devidx" ] && echo " ${devidx}GHz")'
	option isolate 1
	...
	option macaddr	$(generate_macaddr 2)

config wifi-iface-off wcli$devidx
	option ifname	wcli$devidx
	option device   radio$devidx
	option network  wan
	option mode     sta
	option ssid     default
	option disabled	1
	option macaddr	$(generate_macaddr 3)
