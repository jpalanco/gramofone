...
ifeq ($(CONFIG_TARGET_INITRAMFS_MTD_ROOTFS_NO_SPLIT),y)
  define mtd_split_rootfs
	sed -i 's/.*CONFIG_MTD_ROOTFS_SPLIT.*/# CONFIG_MTD_ROOTFS_SPLIT is not set/' $(LINUX_DIR)/.config
  endef
else
  define mtd_split_rootfs
		sed -i 's/.*CONFIG_MTD_ROOTFS_SPLIT.*/CONFIG_MTD_ROOTFS_SPLIT=y/' $(LINUX_DIR)/.config
  endef
endif
