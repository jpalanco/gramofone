...
struct ag71xx *athr_gmac[2] = {NULL, NULL};
...
#define MAC_CFG1_INIT	(MAC_CFG1_RXE | MAC_CFG1_TXE | \
			 MAC_CFG1_SRX | MAC_CFG1_STX | MAC_CFG1_TFC | MAC_CFG1_RFC)
...
int ag71xx_s27_ioctl(struct ag71xx *ag,struct ifreq *ifr, int cmd)
{
	struct eth_cfg_params *etd;
	if (NULL == ifr) return -EINVAL;  
	if(cmd  == S26_RD_PHY) {
		etd = (struct eth_cfg_params *)ifr->ifr_data;
            printk("S26_RD_PHY:etd->portnum=%d etd->phy_reg=%d\n", etd->portnum, etd->phy_reg);
			if(etd->portnum != 0xf)
				etd->val = ar7240sw_phy_read(ag->mii_bus, etd->portnum, etd->phy_reg);
			else
				etd->val = ar71xx_sw_reg_read(ag->mii_bus, etd->phy_reg);
	}
	else if(cmd  == S26_WR_PHY) {
		etd = (struct eth_cfg_params *)ifr->ifr_data;
            printk("S26_WR_PHY:etd->portnum=%d etd->phy_reg=%d etd->val=0X%X\n", etd->portnum, etd->phy_reg, etd->val);
			if(etd->portnum != 0xf)
				ar7240sw_phy_write(ag->mii_bus, etd->portnum, etd->phy_reg, etd->val);
			else
				ar71xx_sw_reg_write(ag->mii_bus, etd->phy_reg, etd->val);
	}
	else
	{
		return -EINVAL;
...
int ag71xx_mii_ioctl(struct phy_device *phy_dev,struct ifreq *ifr, int cmd)
{
	struct eth_cfg_params *etd;
	int i=0,j=0;
	u_int32_t *rxdrops;
	...
	if(cmd  == S26_RD_PHY) {
		etd = (struct eth_cfg_params *)ifr->ifr_data;
            printk("S26_RD_PHY:etd->portnum=%d etd->phy_reg=%d\n", etd->portnum, etd->phy_reg);
			if(etd->portnum != 0xf)
				etd->val = ar7240sw_phy_read(phy_dev->bus, etd->portnum, etd->phy_reg);
			else
				etd->val = ar71xx_sw_reg_read(phy_dev->bus, etd->phy_reg);
	}
	else if(cmd  == S26_WR_PHY) {
		etd = (struct eth_cfg_params *)ifr->ifr_data;
            printk("S26_WR_PHY:etd->portnum=%d etd->phy_reg=%d etd->val=0X%X\n", etd->portnum, etd->phy_reg, etd->val);
			if(etd->portnum != 0xf)
				ar7240sw_phy_write(phy_dev->bus, etd->portnum, etd->phy_reg, etd->val);
			else
				ar71xx_sw_reg_write(phy_dev->bus, etd->phy_reg, etd->val);
	}
	else
	{
		return -EINVAL;
...
static int ag71xx_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	case S26_WR_PHY:
		if (ag->phy_dev == NULL){
			return ag71xx_s27_ioctl(ag, ifr, cmd);
			}
		else{
			return ag71xx_mii_ioctl(ag->phy_dev, ifr, cmd);			
			}
...
static int __devinit ag71xx_probe(struct platform_device *pdev)
{
	...
	if (soc_is_ar9341())
	{
		if (1 == pdev->id)
		{
	    	athr_gmac[0]->p_ag_data = (void *)athr_gmac[1];
	    	athr_gmac[1]->p_ag_data = (void *)athr_gmac[0];
			err = ag71xx_phy_connect(ag);
			if (err)
				goto err_unregister_netdev;
			err = ag71xx_phy_connect(athr_gmac[0]);
			if (err)
				goto err_unregister_netdev;
		}
	}
