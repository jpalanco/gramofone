...
instance_start(struct service_instance *in)
{
	...
	syslog(LOG_INFO, "Started instance %s::%s "
					"[pid = %d] [respawn = %s]", in->srv->name, in->name,
					pid, in->respawn?"true":"false");
	in->proc.pid = pid;
...
instance_exit(struct uloop_process *p, int ret)
{
	...
	syslog(LOG_DEBUG, "Instance %s::%s exit with error code %d after %ld seconds\n", in->srv->name, in->name, ret, runtime);
	//if (upgrade_running)
	//	return;
	...
	if (in->halt) {
		...
		if (in->respawn_count > in->respawn_retry && in->respawn_retry > 0 ) {
			syslog(LOG_INFO, "Instance %s::%s is in a crash loop %d crashes, %ld seconds since last crash\n", in->srv->name, in->name, in->respawn_count, runtime);
...
instance_config_parse(struct service_instance *in)
{
	...
	if (tb[INSTANCE_ATTR_RESPAWN]) {
		int i = 0;
		uint32_t vals[3] = { instance_default_respawn_timeout,
				instance_default_respawn_threshold,
				instance_default_respawn_retry };
